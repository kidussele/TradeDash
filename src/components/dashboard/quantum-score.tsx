...
export function QuantumScore({ entries }: QuantumScoreProps) {
  const { kilaScore } = useMemo(() => {
    ...
    // 4. Consistency (Sharpe Ratio, normalized) - handle low trade count
    const pnlValues = closedTrades.map(t => t.pnl || 0);
    const meanPnl = pnlValues.reduce((a, b) => a + b, 0) / pnlValues.length;
    const stdDev = pnlValues.length > 1 ? Math.sqrt(pnlValues.map(x => Math.pow(x - meanPnl, 2)).reduce((a, b) => a + b) / pnlValues.length) : 0;
    const sharpeRatio = stdDev > 0 ? meanPnl / stdDev : (meanPnl > 0 ? 2 : 0); // Give a positive boost if single trade is a winner
    ...
    const profitFactor = grossLoss > 0 ? grossProfit / grossLoss : grossProfit > 0 ? 5 : 0; // Use a higher reward for no losses
  ...
  const hasData = (entries || []).filter(e => e.result !== 'Ongoing' && e.pnl !== undefined).length > 0;
    ...
    <Card className="flex flex-col h-full">
      ...
      <CardContent className="flex-1 flex flex-col items-center justify-center pb-0">
        {hasData ? (
          <ChartContainer
            config={chartConfig}
            className="mx-auto aspect-square h-full max-w-[250px] relative"
          >
            <RadialBarChart
              data={chartData}
              startAngle={90}
          ...
          <div className="flex-1 flex items-center justify-center text-center text-muted-foreground text-sm p-4">
            No closed trades with P&L data yet.
          </div>
        )}
      </CardContent>
    </Card>
